<li id="user_label" class="nav-item dropdown px-3" title="[[global:header.profile]]">
	...
	<ul id="user-control-list" component="header/usercontrol" class="overscroll-behavior-contain user-dropdown dropdown-menu dropdown-menu-end p-1 text-sm ff-base" role="menu">
		<li>
			<a class="dropdown-item d-flex align-items-center gap-2" component="header/profilelink" href="{relative_path}/user/{user.userslug}" role="menuitem" aria-label="[[user:profile]]">
				<span component="user/status" class="flex-shrink-0 oe-status-dot status {user.status}"><span class="visually-hidden">[[global:{user.status}]]</span></span>
		...
		<li>
			<a href="#" class="dropdown-item user-status d-flex align-items-center gap-2 {{{ if user.online }}}selected{{{ end }}}" data-status="online" role="menuitem">
				<span component="user/status" class="flex-shrink-0 oe-status-dot status online"></span>
				<span class="flex-grow-1">[[global:online]]</span>
			</a>
		</li>
		<li>
			<a href="#" class="dropdown-item user-status d-flex align-items-center gap-2 {{{ if user.away }}}selected{{{ end }}}" data-status="away" role="menuitem">
				<span component="user/status" class="flex-shrink-0 oe-status-dot status away"></span>
				<span class="flex-grow-1">[[global:away]]</span>
			</a>
		</li>
		<li>
			<a href="#" class="dropdown-item user-status d-flex align-items-center gap-2 {{{ if user.dnd }}}selected{{{ end }}}" data-status="dnd" role="menuitem">
				<span component="user/status" class="flex-shrink-0 oe-status-dot status dnd"></span>
				<span class="flex-grow-1">[[global:dnd]]</span>
			</a>
		</li>
		<li>
			<a href="#" class="dropdown-item user-status d-flex align-items-center gap-2 {{{ if user.offline }}}selected{{{ end }}}" data-status="offline" role="menuitem">
				<span component="user/status" class="flex-shrink-0 oe-status-dot status offline"></span>

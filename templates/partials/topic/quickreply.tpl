...
<div component="topic/quickreply/container" class="quick-reply d-flex gap-3 mb-4">
	<div class="icon hidden-xs">
		<a class="d-inline-block position-relative" href="{{{ if loggedInUser.userslug }}}{config.relative_path}/user/{loggedInUser.userslug}{{{ else }}}#{{{ end }}}">
			...
			{{{ if loggedInUser.status }}}<span component="user/status" class="position-absolute top-100 start-100 oe-status-dot status {loggedInUser.status}"><span class="visually-hidden">[[global:{loggedInUser.status}]]</span></span>{{{ end }}}
		</a>
	</div>
	<form class="flex-grow-1 d-flex flex-column gap-2" method="post" action="{config.relative_path}/compose">
		...
		<div>
			<div class="d-flex justify-content-end gap-2">
				<button type="button" component="topic/quickreply/upload/button" class="btn btn-ghost btn-sm"><i class="fa fa-upload"></i></button>
				<button type="button" component="topic/quickreply/expand" class="btn btn-ghost btn-sm" title="[[topic:open-composer]]"><i class="fa fa-expand"></i></button>

<button type="button" class="btn btn-ghost btn-sm ff-secondary d-flex align-items-center gap-2 dropdown-toggle oe-control-button" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
	<span component="category-selector-selected">
		{{{ if (selectedCategory && !showCategorySelectLabel) }}}
		<span class="category-item d-inline-flex align-items-center gap-1">
			{{{ if selectedCategory.icon }}}
			<span class="oe-category-inline-icon" aria-hidden="true">
				<i class="fa {selectedCategory.icon}"></i>
			</span>
			{{{ end }}}

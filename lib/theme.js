'use strict';

const path = require('path');

const meta = require.main.require('./src/meta');
const nconf = require.main.require('nconf');
const groups = require.main.require('./src/groups');

const theme = module.exports;
const THEME_SETTINGS_KEY = 'obsidian-exchange';
const THEME_PLUGIN_ID = 'nodebb-theme-obsidian-exchange';
const THEME_ICON_PREFIX = `/plugins/${THEME_PLUGIN_ID}/icons/`;
const IMAGE_EXTENSION_PATTERN = /\.(png|gif|jpe?g|webp|svg)(\?|#|$)/i;

function normalizeGroupKey(value) {
	return String(value || '').trim().toLowerCase();
}

function sanitizeMappingKey(value) {
	const normalized = normalizeGroupKey(value);
	return normalized || null;
}

function encodeIconSrc(value) {
	return encodeURI(value).replace(/#/g, '%23');
}

function decodeHtmlEntities(value) {
	return String(value || '')
		.replace(/&#x([0-9a-f]+);/gi, (match, hex) => {
			const parsed = parseInt(hex, 16);
			return Number.isFinite(parsed) ? String.fromCodePoint(parsed) : match;
		})
		.replace(/&#([0-9]+);/g, (match, number) => {
			const parsed = parseInt(number, 10);
			return Number.isFinite(parsed) ? String.fromCodePoint(parsed) : match;
		})
		.replace(/&quot;/g, '"')
		.replace(/&#39;/g, '\'')
		.replace(/&amp;/g, '&')
		.replace(/&lt;/g, '<')
		.replace(/&gt;/g, '>');
}

function resolveMappedIconSrc(rawValue) {
	if (typeof rawValue !== 'string') {
		return null;
	}

	let value = decodeHtmlEntities(rawValue).trim();
	if (!value) {
		return null;
	}

	if (/^https?:\/\//i.test(value) || value.startsWith('//') || value.startsWith('/')) {
		return encodeIconSrc(value);
	}

	value = value.replace(/\\/g, '/').replace(/^\.\//, '');
	if (!value || value.includes('..')) {
		return null;
	}

	if (value.startsWith('icons/')) {
		return encodeIconSrc(`${THEME_ICON_PREFIX}${value.slice('icons/'.length)}`);
	}

	if (value.startsWith('plugins/') || value.startsWith('assets/') || value.startsWith('uploads/') || value.startsWith('api/')) {
		return encodeIconSrc(`/${value}`);
	}

	if (value.indexOf('/') === -1) {
		return encodeIconSrc(`${THEME_ICON_PREFIX}${value}`);
	}

	return encodeIconSrc(`/${value}`);
}

function parseObjectMappings(parsedValue) {
	const mappings = new Map();

	if (!parsedValue || typeof parsedValue !== 'object' || Array.isArray(parsedValue)) {
		return mappings;
	}

	Object.entries(parsedValue).forEach(([groupName, iconPath]) => {
		const key = sanitizeMappingKey(groupName);
		const src = resolveMappedIconSrc(iconPath);
		if (!key || !src) {
			return;
		}

		mappings.set(key, src);
	});

	return mappings;
}

function parseLineMappings(rawValue) {
	const mappings = new Map();

	rawValue.split(/\r?\n/).forEach((line) => {
		const trimmed = line.trim();
		if (!trimmed || trimmed.startsWith('#') || trimmed.startsWith('//')) {
			return;
		}

		const separatorMatch = trimmed.match(/^([^:=]+?)\s*(?:=|:)\s*(.+)$/);
		if (!separatorMatch) {
			return;
		}

		const key = sanitizeMappingKey(separatorMatch[1]);
		const src = resolveMappedIconSrc(separatorMatch[2]);
		if (!key || !src) {
			return;
		}

		mappings.set(key, src);
	});

	return mappings;
}

async function getGroupIconMappings() {
	const settings = await meta.settings.get(THEME_SETTINGS_KEY);
	const rawMappings = typeof settings.groupIconMappings === 'string' ? settings.groupIconMappings.trim() : '';

	if (!rawMappings) {
		return new Map();
	}

	try {
		const parsed = JSON.parse(rawMappings);
		const objectMappings = parseObjectMappings(parsed);
		if (objectMappings.size) {
			return objectMappings;
		}
	} catch (err) {
		// Fall through to simple line-based parsing for ACP convenience.
	}

	return parseLineMappings(rawMappings);
}

function serializeMappings(mappings) {
	if (!(mappings instanceof Map) || !mappings.size) {
		return {};
	}

	return Object.fromEntries(mappings.entries());
}

function extractGroupMapping(group, mappings) {
	const lookupKeys = [
		group && group.name,
		group && group.displayName,
		group && group.slug,
		group && group.systemName,
	];

	for (const key of lookupKeys) {
		const normalized = sanitizeMappingKey(key);
		if (!normalized) {
			continue;
		}

		const mappedSrc = mappings.get(normalized);
		if (mappedSrc) {
			return mappedSrc;
		}
	}

	return null;
}

function isImageToken(token) {
	return (
		/^https?:\/\//i.test(token) ||
		token.startsWith('//') ||
		token.startsWith('/') ||
		IMAGE_EXTENSION_PATTERN.test(token)
	);
}

function mergeGroupIconTokens(iconValue, mappedSrc) {
	const tokens = String(iconValue || '')
		.split(/\s+/)
		.filter(Boolean)
		.filter(token => !isImageToken(token));

	const hasFallbackIcon = tokens.some(token => token === 'fa' || token.startsWith('fa-') || /^fa[srlbd]$/.test(token));
	if (!hasFallbackIcon) {
		tokens.unshift('fa-shield');
	}

	tokens.push(mappedSrc);
	return tokens.join(' ');
}

function isGroupLike(value) {
	return Boolean(
		value &&
		typeof value === 'object' &&
		typeof value.name === 'string' &&
		(
			typeof value.slug === 'string' ||
			typeof value.displayName === 'string' ||
			typeof value.icon === 'string' ||
			typeof value.userTitleEnabled !== 'undefined'
		)
	);
}

function applyGroupIconMappings(value, mappings, seen = new WeakSet()) {
	if (Array.isArray(value)) {
		value.forEach(item => applyGroupIconMappings(item, mappings, seen));
		return value;
	}

	if (!value || typeof value !== 'object') {
		return value;
	}

	if (seen.has(value)) {
		return value;
	}
	seen.add(value);

	if (isGroupLike(value)) {
		const mappedSrc = extractGroupMapping(value, mappings);
		if (mappedSrc) {
			value.icon = mergeGroupIconTokens(value.icon, mappedSrc);
			value.imageIcon = mappedSrc;
		}
	}

	Object.values(value).forEach(child => applyGroupIconMappings(child, mappings, seen));

	return value;
}

function humanizeGroupName(value) {
	return String(value || '')
		.trim()
		.split(/\s+/)
		.filter(Boolean)
		.map(word => word.charAt(0).toUpperCase() + word.slice(1))
		.join(' ');
}

function sanitizeRoleLabel(value) {
	const stripped = String(value || '')
		.replace(/<[^>]*>/g, ' ')
		.replace(/\s+/g, ' ')
		.trim();

	if (!stripped) {
		return '';
	}

	if (/[A-Z]/.test(stripped) || stripped.includes(' ')) {
		return stripped;
	}

	return humanizeGroupName(stripped.replace(/[-_]+/g, ' '));
}

function getPrimaryRoleRawTitle(userData, isAdministrator) {
	const titleArray = Array.isArray(userData && userData.groupTitleArray) ?
		userData.groupTitleArray.filter(Boolean) :
		[];
	let groupTitle = userData && userData.groupTitle;
	if (!titleArray.length && typeof groupTitle === 'string' && groupTitle.trim().startsWith('[')) {
		try {
			const parsed = JSON.parse(groupTitle);
			if (Array.isArray(parsed) && parsed[0]) {
				groupTitle = parsed[0];
			}
		} catch (err) {
			// Keep the raw string if groupTitle is not valid JSON.
		}
	}
	return titleArray[0] ||
		groupTitle ||
		userData.userTitle ||
		(isAdministrator ? 'administrators' : '');
}

function resolveRoleIconSrc(rawTitle, roleLabel, mappings) {
	if (!mappings || !mappings.size) {
		return '';
	}

	const lookupCandidates = [rawTitle, roleLabel];
	for (const candidate of lookupCandidates) {
		const key = sanitizeMappingKey(candidate);
		if (!key) {
			continue;
		}

		const mappedSrc = mappings.get(key);
		if (mappedSrc) {
			return mappedSrc;
		}
	}

	return '';
}

function extractImageSrcFromIconToken(token) {
	if (typeof token !== 'string') {
		return '';
	}

	const value = decodeHtmlEntities(token).trim();
	if (!value) {
		return '';
	}

	if (isImageToken(value) && !value.startsWith('fa-')) {
		return resolveMappedIconSrc(value) || '';
	}

	if (value.startsWith('fa-')) {
		const stripped = value.slice(3);
		if (isImageToken(stripped)) {
			return resolveMappedIconSrc(stripped) || '';
		}
	}

	return '';
}

function extractImageSrcFromIconValue(iconValue) {
	return String(iconValue || '')
		.split(/\s+/)
		.map(extractImageSrcFromIconToken)
		.find(Boolean) || '';
}

function getPrimaryRoleGroup(userData, userGroups, isAdministrator) {
	if (!Array.isArray(userGroups) || !userGroups.length) {
		return null;
	}

	const titleOrder = Array.isArray(userData && userData.groupTitleArray) ?
		userData.groupTitleArray.filter(Boolean) :
		[];
	const fallbackTitles = [getPrimaryRoleRawTitle(userData, isAdministrator)];
	const candidates = titleOrder.concat(fallbackTitles).map(sanitizeMappingKey).filter(Boolean);

	for (const candidate of candidates) {
		const match = userGroups.find((group) => {
			const groupKeys = [
				group && group.name,
				group && group.displayName,
				group && group.userTitle,
				group && group.userTitleEscaped,
			].map(sanitizeMappingKey).filter(Boolean);

			return groupKeys.includes(candidate);
		});

		if (match) {
			return match;
		}
	}

	if (isAdministrator) {
		const administratorsGroup = userGroups.find(group => sanitizeMappingKey(group && group.name) === 'administrators');
		if (administratorsGroup) {
			return administratorsGroup;
		}
	}

	return userGroups[0] || null;
}

function resolveRoleBadgeImageSrc(primaryRoleGroup, rawTitle, roleLabel, mappings) {
	const groupImageSrc = resolveMappedIconSrc(primaryRoleGroup && primaryRoleGroup.imageIcon);
	if (groupImageSrc) {
		return groupImageSrc;
	}

	const extractedIconSrc = extractImageSrcFromIconValue(primaryRoleGroup && primaryRoleGroup.icon);
	if (extractedIconSrc) {
		return extractedIconSrc;
	}

	return resolveRoleIconSrc(rawTitle, roleLabel, mappings);
}

theme.defineWidgetAreas = async function (areas) {
	const locations = ['header', 'sidebar', 'footer'];
	const templates = [
		'categories.tpl', 'category.tpl', 'topic.tpl', 'users.tpl',
		'unread.tpl', 'recent.tpl', 'popular.tpl', 'top.tpl', 'tags.tpl', 'tag.tpl',
		'login.tpl', 'register.tpl',
	];
	function capitalizeFirst(str) {
		return str.charAt(0).toUpperCase() + str.slice(1);
	}
	templates.forEach((template) => {
		locations.forEach((location) => {
			areas.push({
				name: `${capitalizeFirst(template.split('.')[0])} ${capitalizeFirst(location)}`,
				template: template,
				location: location,
			});
		});
	});

	areas = areas.concat([
		{
			name: 'Account Header',
			template: 'account/profile.tpl',
			location: 'header',
		},
	]);
	return areas;
};

theme.preinit = function (params, callback) {
	nconf.set('base_templates_path', path.join(nconf.get('themes_path'), 'nodebb-theme-persona/templates'));

	callback();
};

theme.init = function (params, callback) {
	const routeHelpers = require.main.require('./src/routes/helpers');
	routeHelpers.setupAdminPageRoute(params.router, '/admin/plugins/obsidian-exchange', function (req, res) {
		res.render('admin/plugins/obsidian-exchange', {
			title: 'Obsidian Exchange Theme',
		});
	});

	callback();
};

theme.addAdminNavigation = function (header, callback) {
	header.plugins.push({
		route: '/plugins/obsidian-exchange',
		icon: 'fa-paint-brush',
		name: 'Obsidian Exchange Theme',
	});

	callback(null, header);
};

theme.getConfig = async function (config) {
	const [settings, groupIconMappings, administratorGroup] = await Promise.all([
		meta.settings.get(THEME_SETTINGS_KEY),
		getGroupIconMappings(),
		groups.getGroupData('administrators'),
	]);
	config.disableMasonry = settings.disableMasonry === 'on';
	config.enableQuickReply = settings.enableQuickReply === 'on';
	config.obsidianGroupIconMappings = serializeMappings(groupIconMappings);
	config.obsidianAdministratorRoleIcon = resolveRoleBadgeImageSrc(
		administratorGroup,
		'administrators',
		'Administrators',
		groupIconMappings
	);
	return config;
};

theme.addUserListFields = async function (hookData) {
	if (!Array.isArray(hookData.fields)) {
		hookData.fields = [];
	}

	if (!hookData.fields.includes('groupTitle')) {
		hookData.fields.push('groupTitle');
	}

	if (!hookData.fields.includes('userTitle')) {
		hookData.fields.push('userTitle');
	}

	return hookData;
};

theme.decorateUserList = async function (hookData) {
	if (!hookData || !Array.isArray(hookData.users) || !hookData.users.length) {
		return hookData;
	}

	const users = hookData.users.filter(Boolean);
	const uids = users
		.map(user => parseInt(user && user.uid, 10))
		.filter(uid => Number.isInteger(uid) && uid > 0);
	const [isAdminFlags, userGroupsList, groupIconMappings] = await Promise.all([
		uids.length ? groups.isMembers(uids, 'administrators') : [],
		uids.length ? groups.getUserGroups(uids) : [],
		getGroupIconMappings(),
	]);
	const uidToAdmin = new Map();
	const uidToGroups = new Map();

	uids.forEach((uid, index) => {
		uidToAdmin.set(String(uid), Boolean(isAdminFlags[index]));
		uidToGroups.set(String(uid), Array.isArray(userGroupsList[index]) ? userGroupsList[index] : []);
	});

	users.forEach((userData) => {
		const isAdministrator = uidToAdmin.get(String(userData.uid)) || false;
		const userGroups = uidToGroups.get(String(userData.uid)) || [];
		const rawRoleTitle = getPrimaryRoleRawTitle(userData, isAdministrator);
		const primaryRoleLabel = sanitizeRoleLabel(rawRoleTitle);
		const primaryRoleGroup = getPrimaryRoleGroup(userData, userGroups, isAdministrator);
		const primaryRoleIcon = resolveRoleBadgeImageSrc(
			primaryRoleGroup,
			rawRoleTitle,
			primaryRoleLabel,
			groupIconMappings
		);

		userData.isAdministrator = isAdministrator;
		userData.primaryGroupTitle = primaryRoleLabel;
		userData.primaryRoleLabel = primaryRoleLabel;
		userData.primaryRoleIcon = primaryRoleIcon;
	});

	hookData.users = hookData.users
		.map((userData, index) => ({ userData, index }))
		.sort((left, right) => {
			const rightIsAdmin = Number(Boolean(right.userData && right.userData.isAdministrator));
			const leftIsAdmin = Number(Boolean(left.userData && left.userData.isAdministrator));
			const adminDiff = rightIsAdmin - leftIsAdmin;
			if (adminDiff !== 0) {
				return adminDiff;
			}

			return left.index - right.index;
		})
		.map(entry => entry.userData);

	return hookData;
};

theme.injectGroupIcons = async function (hookData) {
	const mappings = await getGroupIconMappings();
	if (!mappings.size) {
		return hookData;
	}

	return applyGroupIconMappings(hookData, mappings);
};
